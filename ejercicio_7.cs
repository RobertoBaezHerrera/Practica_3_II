...
public class ejercicio_7 : MonoBehaviour
{
    ...
    void Update()
    {
        // Cubo rota hacia la esfera
        cubo.transform.LookAt(esfera.transform.position);

        // Cubo sigue a esfera
        Vector3 direccion_cubo_a_esfera = esfera.transform.position - cubo.transform.position;
        direccion_cubo_a_esfera.y = 0f;
        cubo.transform.Translate(direccion_cubo_a_esfera.normalized * speed * Time.deltaTime, Space.World);
